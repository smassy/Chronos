        ...
        <table id="scheduler">
            ...
            <tbody id="<?= $availability[0]["slot_time"]->format('Y-m-d') ?>">
                <?php foreach ($availability as $slot): ?>
                    <tr>
                        <td id="slot-<?= $slot['slot_time']->format('Hi') ?>"><?= $slot['slot_time']->format('H:i') ?> 
                        </td>

                        <?php if (!$slot['booked']): ?>
                            <td class="first-party free">
                        </td>
                        <?php elseif (isset($slot['slots'])): ?>
                            <td rowspan="<?= $slot['slots'] ?>" class="first-party booked"><?= $slot['title'] ?>
                        </td>
                        <?php endif; ?>
                        <td>
                        </td>
